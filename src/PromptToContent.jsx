...
.cover { background: linear-gradient(135deg, #7C3AED, #4F46E5); color: #fff; padding: 60px 50px; }
...
.meta-item { background: #fff; border-radius: 8px; padding: 12px 16px; border-left: 3px solid #7C3AED; }
...
.content-body h1, .content-body h2, .content-body h3 { color: #7C3AED; margin: 20px 0 10px; }
...
<div class="schema-section"><h3 style="margin-bottom:12px;color:#7C3AED">Schema Markup</h3><pre>${output.schema.replace(/</g,"&lt;")}</pre></div>
        ...
        {step === 1 && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {AUDIENCES.map(a => (
                      <div key={a} onClick={() => setAudience(a)}
                        style={{ padding: "4px 12px", borderRadius: 20, fontSize: 11, cursor: "pointer", fontWeight: audience === a ? 600 : 400, background: audience === a ? "#7C3AED22" : bg3, color: audience === a ? "#A78BFA" : txt2, border: `1px solid ${audience === a ? "#7C3AED44" : bdr}` }}>
                        {a}
                      </div>
                    ))}
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {PAGE_TYPES.map(pt => (
                    <div key={pt.id} onClick={() => setPageType(pt.id)}
                      style={{ padding: "10px 12px", borderRadius: 8, border: `1px solid ${pageType === pt.id ? "#7C3AED44" : bdr}`, background: pageType === pt.id ? "#7C3AED11" : bg3, cursor: "pointer" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                        <span style={{ fontSize: 16 }}>{pt.icon}</span>
                        <span style={{ fontSize: 12, fontWeight: pageType === pt.id ? 600 : 400, color: pageType === pt.id ? "#A78BFA" : txt }}>{pt.label}</span>
                      </div>
              ...
              <div style={{ background: "#7C3AED11", border: "1px solid #7C3AED33", borderRadius: 12, padding: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: "#A78BFA", marginBottom: 10 }}>⚡ AI will generate:</div>
                {[
              ...
              <button onClick={generate} disabled={loading || !topic.trim()}
                style={{ padding: "14px", borderRadius: 12, border: "none", background: loading || !topic.trim() ? "#333" : "linear-gradient(135deg, #7C3AED, #4F46E5)", color: loading || !topic.trim() ? txt3 : "#fff", fontWeight: 700, fontSize: 15, cursor: loading || !topic.trim() ? "not-allowed" : "pointer" }}>
        ...
        {step === 2 && (
          <div style={{ background: bg2, border: `1px solid ${bdr}`, borderRadius: 12, padding: 60, textAlign: "center" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, maxWidth: 320, margin: "0 auto" }}>
              {[
                { label: "Writing full content", color: "#7C3AED" },
        ...
        {step === 3 && output && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 10, marginBottom: 16 }}>
              {[
                { label: "Words",       value: output.wordCount,    color: "#7C3AED" },
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
              ...
              <button onClick={exportPDF}
                style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: "#7C3AED", color: "#fff", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                📥 Export PDF
              </button>
            </div>
            ...
            {activeTab === "meta" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                {/* HTML Meta Code */}
                <div style={{ background: bg2, border: "1px solid #7C3AED44", borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ padding: "10px 16px", background: "#7C3AED11", borderBottom: "1px solid #7C3AED22", display: "flex", justifyContent: "space-between" }}>
                    ...
                    <button onClick={() => copyText(`<title>${output.title}</title>\n<meta name="description" content="${output.metaDesc}">\n<meta property="og:title" content="${output.ogTitle}">\n<meta property="og:description" content="${output.ogDesc}">`, "html")}
                      style={{ padding: "3px 10px", borderRadius: 6, border: "1px solid #7C3AED44", background: "transparent", color: copied === "html" ? "#059669" : "#A78BFA", fontSize: 11, cursor: "pointer" }}>
                      {copied === "html" ? "✅" : "📋 Copy"}
                    </button>
                  </div>
