...
export default function App() {
  ...
  async function runPageSpeed(url) {
    ...
    try {
      ...
      const score = (d, key) => Math.round((d.lighthouseResult?.categories?.[key]?.score||0)*100);
      const val   = (d, key) => d.lighthouseResult?.audits?.[key]?.displayValue || "N/A";
...
LCP (Load):     ${val(mob,"largest-contentful-paint")}  ${val(mob,"largest-contentful-paint")==="N/A"?"":"(target: <2.5s)"}
TBT (Input):    ${val(mob,"total-blocking-time")}  ${val(mob,"total-blocking-time")==="N/A"?"":"(target: <100ms)"}
CLS (Visual):   ${val(mob,"cumulative-layout-shift")}  ${val(mob,"cumulative-layout-shift")==="N/A"?"":"(target: <0.1)"}
FCP (Paint):    ${val(mob,"first-contentful-paint")}
...
${(mob.lighthouseResult?.audits?.["render-blocking-resources"]?.score||1)<1?"• Fix render-blocking resources\n":""}${(mob.lighthouseResult?.audits?.["uses-optimized-images"]?.score||1)<1?"• Optimize images\n":""}${(mob.lighthouseResult?.audits?.["unused-javascript"]?.score||1)<1?"• Remove unused JavaScript\n":""}${(mob.lighthouseResult?.audits?.["unused-css-rules"]?.score||1)<1?"• Remove unused CSS\n":""}${(mob.lighthouseResult?.audits?.["uses-text-compression"]?.score||1)<1?"• Enable text compression\n":""}`;
      ...
      setCount(newCount);
      localStorage.setItem("seo_count", newCount);
    } catch(e) {
      addMsg(tool.id, { role:"assistant", text:"Error: "+e.message });
    }
    setLoading(false);
  }
  ...
  const s = {
    ...
    catBtn: a => ({ padding:"3px 9px", borderRadius:20, fontSize:11, fontWeight: a?600:400, cursor:"pointer", border:"1px solid", background: a?"#7C3AED22":"transparent", color: a?"#A78BFA":"#555", borderColor: a?"#7C3AED44":"#222", transition:"all 0.1s" }),
    secLabel: { fontSize:10, color:"#333", padding:"8px 8px 3px", textTransform:"uppercase", letterSpacing:"0.08em" },
    navItem: (a, color) => ({ display:"flex", alignItems:"center", gap:8, padding:"7px 10px", borderRadius:8, marginBottom:1, cursor:"pointer", fontSize:12, fontWeight: a?600:400, background: a?color+"22":"transparent", color: a?color:"#666", border: a?`1px solid ${color}33`:"1px solid transparent", whiteSpace:"nowrap", transition:"all 0.1s" }),
    ...
    aBub:   { alignSelf:"flex-start", background:"#1a1a1a", border:"1px solid #2a2a2a", color:"#e8e8e8", padding:"12px 14px", borderRadius:"4px 12px 12px 12px", maxWidth:"88%", fontSize:13, lineHeight:1.75, whiteSpace:"pre-wrap", position:"relative" },
    ...
    runBtn: ok => ({ padding:"0 18px", borderRadius:10, border:"none", background: ok?"#7C3AED":"#222", color: ok?"#fff":"#444", fontWeight:600, fontSize:13, cursor: ok?"pointer":"not-allowed", flexShrink:0, minWidth:64, transition:"background 0.2s" }),
    ...
    <div style={s.app}>
      ...
      <div style={s.side}>
        <div style={s.logo}>
          ...
          <div>
            ...
            <div style={{ fontSize:10, color:"#444" }}>v3.0 · {TOOLS.length} tools</div>
          </div>
        </div>
        <div style={s.nav}>
          <div style={s.catRow}>
          ...
          {catGroups.map(c => (
            <div key={c}>
              ...
              {filtered.filter(t=>t.cat===c).map(t => (
                <div key={t.id} style={s.navItem(tool.id===t.id, t.color)} onClick={()=>setTool(t)}>
      ...
      <div style={s.main}>
        ...
        <div style={s.header}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:"#444" }}>{tool.cat} · {curMsgs.filter(m=>m.role==="user").length} queries</div>
            </div>
          </div>
          <div style={{ display:"flex", gap:6, alignItems:"center" }}>
            {!tool.isApi && Object.entries(MODELS).map(([k,v]) => (
              <div key={k} onClick={()=>setModel(k)} style={{ padding:"4px 12px", borderRadius:20, cursor:"pointer", fontSize:12, fontWeight: model===k?700:400, background: model===k?v.color+"22":"transparent", color: model===k?v.color:"#444", border:`1px solid ${model===k?v.color+"55":"#222"}`, transition:"all 0.1s" }}>
                {v.name}
              </div>
            ))}
            {curMsgs.length > 0 && (
        ...
        <div style={s.msgs}>
          ...
          {curMsgs.map((m, i) => (
            <div key={i} style={m.role==="user" ? { display:"flex", justifyContent:"flex-end" } : { display:"flex", justifyContent:"flex-start" }}>
              {m.role==="user" ? (
          ...
          {loading && (
            <div style={{ display:"flex", justifyContent:"flex-start" }}>
              <div style={{ ...s.aBub, color:"#444" }}>
                {tool.isApi ? "⚡ Fetching PageSpeed data..." : `🤔 Analyzing with ${MODELS[model]?.name}...`}
              </div>
            </div>
          )}
          <div ref={bottomRef} />
        ...
        <div style={s.inputArea}>
          ...
          <div style={{ fontSize:11, color:"#333", marginTop:5 }}>
            Enter to run · Shift+Enter new line · {tool.isApi?"Google API":`Model: ${MODELS[model]?.name}`} · {TOOLS.length} tools total
          </div>
        </div>
      </div>
      ...
      {showSettings && (
        <div style={s.overlay} onClick={()=>setShowSettings(false)}>
          <div style={s.modal} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ fontSize:12, color:"#555", marginTop:4, marginBottom:8 }}>Keys saved in browser — persist across sessions</div>
            ...
            <label style={s.label}>Google APIs Key — PageSpeed (AIza...)</label>
